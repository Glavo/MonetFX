...
public abstract class ExpressionHelper<T> {
    ...
    public static <T> ExpressionHelper<T> addListener(ExpressionHelper<T> helper, ObservableValue<T> observable, InvalidationListener listener) {
        ...
        return (helper == null) ? new ExpressionHelper.SingleInvalidation<T>(observable, listener) : helper.addListener(listener);
    }
    ...
    public static <T> ExpressionHelper<T> addListener(ExpressionHelper<T> helper, ObservableValue<T> observable, ChangeListener<? super T> listener) {
        ...
        return (helper == null) ? new ExpressionHelper.SingleChange<T>(observable, listener) : helper.addListener(listener);
    }
    ...
    private static final class SingleInvalidation<T> extends ExpressionHelper<T> {
    ...
    private static final class SingleChange<T> extends ExpressionHelper<T> {
    ...
    private static final class Generic<T> extends ExpressionHelper<T> {
                ...
                for (int index = 0; index < invalidationSize; index++) {
                    if (listener.equals(invalidationListeners[index])) {
                        if (invalidationSize == 1) {
                            ...
                            return new ExpressionHelper.SingleInvalidation<T>(observable, invalidationListeners[1 - index]);
                        } else {
